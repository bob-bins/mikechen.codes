...
const initialCellStateFn:CellStateFn = () => Math.random() > .5 ? live : dead
...
const cellState = (time:Integer, row:Integer, col:Integer, {minRow, maxRow, minCol, maxCol}:UniverseDimensions) => {
  row = row < minRow ?
  ...
  const adjacentLiveCount = [
    ...
    [row, col-1], [row, col+1],
    [row+1, col-1], [row+1, col], [row+1, col+1],
  ].reduce((p, [x,y]) => p + getCellState(time-1, x, y, {minRow, maxRow, minCol, maxCol}), 0)
  return getCellState(time-1, row, col, {minRow, maxRow, minCol, maxCol}) == live ?
    ...
    : (adjacentLiveCount == 3 ? live : dead)
}

export const _cellStateCache:{[key:number]:{[key:string]:CellState}} = {}

const setCellState =
  (time:Integer, row:Integer, col:Integer, state:CellState) => {
    if (_cellStateCache[time] == undefined) _cellStateCache[time] = {}
    return _cellStateCache[time][`${row},${col}`] = state
  }

export const getCellState = (time:Integer, row:Integer, col:Integer, {minRow, maxRow, minCol, maxCol}:UniverseDimensions) => {
  row = row < minRow ?
      maxRow+1 - (minRow - row) % (maxRow-minRow) 
    : row > maxRow ?
      minRow-1 + (row - maxRow) % (maxRow-minRow)
  ...
  col = col < minCol ?
    ...
    : col > maxCol ?
      minCol-1 + (col - maxCol) % (maxCol-minCol)
    : col
  return _cellStateCache[time]?.[`${row},${col}`]
    ??
  (
    time == 0
...
const drawGridForDebuggingPurposes =
  (time:Integer, minRow:Integer, minCol:Integer, maxRow:Integer, maxCol:Integer) =>
  {
    range(minRow, maxRow)
      .map(rowIndex =>
        range(minCol, maxCol)
      ...
      .forEach(
        row =>
        console.log(
          row.map(([rowIndex, colIndex]) => getCellState(time, rowIndex, colIndex, {minRow, maxRow, minCol, maxCol})).join(" ")
        )
      )
    console.log('')
    console.log('')
